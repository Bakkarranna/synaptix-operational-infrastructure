...
const ContactSection: React.FC = () => {
  const submitStrategy = useMutation(api.forms.submitStrategy);
  const [formData, setFormData] = useState({ name: '', email: '', websiteUrl: '', businessNeeds: '', bottleneck: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      * End with a strong call to action, encouraging them to book a detailed discovery call. Format this call to action as a markdown link, for example: [Book your free discovery call now](${CALENDLY_LINK}).
      ...
      console.log("AI Strategy Lead data submitted to Convex successfully.");
  ...
  const heading = "Request **Process Mining Audit**";
  const description = "We'll analyze your operational bottlenecks and design a **custom automation blueprint** to eliminate waste and scale revenue.";

  const bottleneckOptions = [
    "High Support Volume (Drowning in tickets)",
    "Slow Sales Follow-up (Leads going cold)",
    "Manual Data Entry (Hours wasted daily)",
        ...
        <div className="bg-white/20 dark:bg-black/20 backdrop-blur-md border border-gray-200 dark:border-white/10 rounded-xl shadow-2xl p-6 sm:p-8 md:p-12 max-w-4xl mx-auto">
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-6" role="form" aria-label="AI Strategy Generation Form">
              ...
              {/* NEW: Bottleneck Selection Dropdown */}
              <div className="group">
                <label htmlFor="bottleneck-select" className="block text-sm font-semibold text-gray-700 dark:text-white/80 mb-2">
                ...
                <select
                  ...
                  className="w-full px-4 py-3 rounded-lg border border-black/10 bg-white/80 dark:bg-black/20 backdrop-blur-sm dark:border-white/10 text-gray-800 dark:text-white focus:ring-2 focus:ring-primary/50 dark:focus:ring-white transition-all"
                  required
                  aria-label="Select your primary operational bottleneck"
                  aria-required="true"
                >
              ...
              <div className="group flex items-start gap-4 px-4 py-3 rounded-lg border border-black/10 bg-white/80 dark:bg-black/20 backdrop-blur-sm dark:border-white/10 transition-all group-focus-within:ring-2 group-focus-within:ring-primary/50 dark:group-focus-within:ring-white">
                ...
                <label htmlFor="contact-business-needs" className="sr-only">Additional context about your operations</label>
                <textarea
                  id="contact-business-needs"
                  name="businessNeeds"
                  placeholder="Additional context: Current team size, monthly revenue, systems in use..."
                  rows={5}
                  ...
                  required
                  aria-label="Provide additional context about your operations"
                  aria-required="true"
                ></textarea>
              </div>
